<%@ page import="reveng.TableDetailSuratJalan" %>
...
<div class="fieldcontain ${hasErrors(bean: tableDetailSuratJalanInstance, field: 'tableHeaderSuratJalan', 'error')} required">
	...
	<g:select id="tableHeaderSuratJalan" name="tableHeaderSuratJalan.id" from="${reveng.TableHeaderSuratJalan.list()}" optionKey="id" required="" value="${tableDetailSuratJalanInstance?.tableHeaderSuratJalan?.id}" class="many-to-one"/>
</div>
